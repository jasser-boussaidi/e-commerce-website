                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        foreach ($stock as $s) {
                            ...
                            echo '<tr>
                                  <th scope="row">' . $i . '</th>
                                  <td>' . $s['products_name'] . '</td>
                                  <td>' . $s['quantite'] . '</td>
                                  <td>
                                    <a data-bs-toggle="modal" data-bs-target="#editCategory' . $s['id'] . '" class="btn btn-success">Modifier</a>
                                  </td>
                                </tr>';
                        }
                        ?>
                        ...
                        foreach ($stock as $s) {
                            echo '
                            <div class="modal fade" id="editCategory' . $s['id'] . '" tabindex="-1" aria-labelledby="editCategoryModalLabel" aria-hidden="true">
                              <div class="modal-dialog">
                                <div class="modal-content">
                                  <div class="modal-header">
                                    <h5 class="modal-title" id="editCategoryModalLabel">Modifier le stock de <span class="text-primary">' . $s['products_name'] . '</span></h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fermer"></button>
                                  </div>
                                  <div class="modal-body">
                                    <form action="modifys.php" method="post">
                                      <input type="hidden" name="id" value="' . $s['id'] . '" /> 
                                      <div class="form-group">
                                        <input type="number" name="quantite" class="form-control" value="' . $s['quantite'] . '" placeholder="Quantité..." required>
                                      </div>
